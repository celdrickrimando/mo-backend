 ...
 *
 * Recurses into tables (tableRows -> tableCells -> each cell's own
 * `content` array of paragraphs) in reading order (row by row, left cell
 * then right cell) — the signatory block (SHORT COMPANY NAME | DLSU-...
 * side by side, "By:" under each) is laid out as a borderless table, so
 * without this recursion its entire contents — every name, every
 * position, every constant-signatory check — would silently never be
 * read at all. A StructuralElement always has exactly one of `paragraph`
...
export async function fetchDocument(docId, accessToken) {
  ...
  let res;
  try {
    // suggestionsViewMode: PREVIEW_SUGGESTIONS_ACCEPTED — without this, the
    // API's default view for a user with edit access (SUGGESTIONS_INLINE)
    // returns each run's textStyle as it was BEFORE any pending suggested
    // edit, even though the Docs UI itself renders a suggested formatting
    // change (e.g. someone applying Ctrl+B while in Suggesting mode) as
    // already bold. That mismatch made a name that visually reads bold in
    // the editor come back as bold: false here, since bold: !!pe.textRun.
    // textStyle?.bold below only ever reads the base style, never
    // suggestedTextStyleChanges. Requesting the accepted-preview view
    // makes the fetched content match what's actually rendered/intended,
    // so every check operates on the same reality a human reviewer sees.
    // (This does mean unresolved suggestions are treated as if already
    // accepted for formatting purposes — a MOA shouldn't be submitted with
    // open suggestions anyway, but that's a separate concern from bold/
    // italic detection and isn't itself checked for here.)
    res = await docs.documents.get({ documentId: docId, suggestionsViewMode: "PREVIEW_SUGGESTIONS_ACCEPTED" });
  } catch (err) {
...
/**
 * Same logic as isTextBold(), but for italics — used for position/rank
 * fields (e.g. "Partnership Head, DLSU-..."), which the template wants
 * italicized rather than bold. Kept as a separate mirrored function
 * rather than a shared {bold, italic} parameter so each call site reads
 * plainly at the point of use (isTextBold(...) / isTextItalic(...))
 * instead of a style-name string that could be typo'd.
 */
export function isTextItalic(runs, needle) {
  let joined = "";
  const runIndexMap = [];
  runs.forEach((run, runIdx) => {
    for (let i = 0; i < run.text.length; i++) runIndexMap.push(runIdx);
  ...
  for (const runIdx of involvedRunIdxs) {
    if (!runs[runIdx].italic) return false;
  }
  return true;
}

const runIndexMapCache = new WeakMap(); // runs array -> its built map, so repeated isRangeBold/isRangeItalic calls against the same runs array (the common case — one /check reuses the same runs array across dozens of checks) don't rebuild it every time

function buildRunIndexMap(runs) {
  const cached = runIndexMapCache.get(runs);
  if (cached) return cached;

  const map = [];
  runs.forEach((run, runIdx) => {
    for (let i = 0; i < run.text.length; i++) map.push(runIdx);
  });
  runIndexMapCache.set(runs, map);
  return map;
}

/**
 * Checks bold/italic at an EXACT fullText character range, rather than
 * re-searching for a piece of text and checking whatever occurrence
 * .indexOf() happens to find first (which is what isTextBold/isTextItalic
 * above do). That distinction matters a lot in practice: a value like an
 * event name or a person's name commonly appears more than once in a MOA
 * (once in a title/preamble, again in a GTC clause, again in the
 * signatory block), often with DIFFERENT formatting in each spot. Any
 * check that already knows exactly where in fullText its match came from
 * (a regex with the 'd'/hasIndices flag, or a manually-tracked offset)
...
const DISMISSIBLE_ISSUE_CODES = {
  ...
  witness_whereof_page_break_needs_manual_check: "wwpagebreak",
  ...
  // Was already exposed as dismissible in popup.js's DISMISSIBLE_ISSUE_TYPES
  // mirror set but missing here — the "Mark resolved" button for this type
  // would have thrown ("isn't a dismissible issue type") the first time
  // anyone actually clicked it. Added to close that gap.
  constant_signatory_position_needs_manual_check: "constsigpos",
};
...
/**
 * "Mark correct — don't flag again", per Mo_Handoff_Notes.md section C.
 *
 * Distinct from DISMISSIBLE_ISSUE_CODES above, which mutes an issue TYPE
 * for the WHOLE document (only appropriate for the 5 "couldn't verify
 * automatically" types, where there's only ever one such note per doc).
 * This is per-INSTANCE: it mutes one specific (issueType, flaggedText)
 * pair, so e.g. confirming one name's bold formatting doesn't hide a real
 * bold violation on a different name elsewhere in the same document.
 *
 * Stored as a list of short hash tokens (not the raw type+text pairs) in
 * a second Drive appProperties value, `moConfirmedCorrect` — same
 * short-code-not-full-string reasoning as `moDismissed`, since Drive
 * appProperties values are capped at ~124 characters each.
 *
 * Self-healing by design: the hash is derived from the exact flagged
 * text, so if that text later changes (formatting actually gets fixed,
 * name gets edited, etc.) the old confirmation naturally stops matching
 * and the check re-evaluates from scratch — no manual invalidation step
 * needed anywhere in this file or in index.js.
 */

// Drive's documented per-value cap for a custom file property.
const DRIVE_PROPERTY_VALUE_MAX_LENGTH = 124;

// Simple non-cryptographic 32-bit hash (FNV-1a), base36-encoded — plenty
// of collision resistance for "don't re-flag this one instance" at the
// scale of issues on a single MOA, and fixed-length regardless of how
// long the flagged text/org name/GTC field is.
export function hashConfirmedPair(issueType, text) {
  const input = `${issueType}\u0000${text ?? ""}`;
  let hash = 0x811c9dc5;
...
/**
 * Reads the set of confirmed-correct (issueType, text) hash tokens for
 * this document. Filtering `runAllChecks()` output against this set
 * happens in index.js, same place `dismissedTypes` filtering already
 * does — this function just does the Drive read.
 */
export async function getConfirmedCorrectHashes(docId, accessToken) {
  const auth = authorizedClient(accessToken);
  const drive = google.drive({ version: "v3", auth });
  const { data } = await drive.files.get({ fileId: docId, fields: "appProperties" });
  const raw = data.appProperties?.moConfirmedCorrect;
  if (!raw) return new Set();
  return new Set(raw.split(",").filter(Boolean));
}

/**
 * Marks one specific flagged instance as confirmed-correct so /check
 * stops re-raising it for this document. Appends to the existing list
 * rather than replacing it. Throws a clear error (rather than silently
 * dropping the oldest entry) if the property value would exceed Drive's
 * per-value size cap — per Mo_Handoff_Notes.md section C point 7.
 */
export async function markIssueCorrect(docId, accessToken, issueType, text) {
  const auth = authorizedClient(accessToken);
  const drive = google.drive({ version: "v3", auth });
  const { data } = await drive.files.get({ fileId: docId, fields: "appProperties" });
  const existingTokens = new Set((data.appProperties?.moConfirmedCorrect || "").split(",").filter(Boolean));

  const token = hashConfirmedPair(issueType, text);
  existingTokens.add(token);
  const newValue = [...existingTokens].join(",");

  if (newValue.length > DRIVE_PROPERTY_VALUE_MAX_LENGTH) {
    throw new Error(
      "This document already has the maximum number of confirmed-correct instances saved. Use \"Reset overrides\" to clear old ones before confirming more."
    );
  }

  await drive.files.update({
    fileId: docId,
    requestBody: { appProperties: { moConfirmedCorrect: newValue } },
  });
...
/**
 * Un-mutes a single previously-confirmed-correct instance (the inverse of
 * markIssueCorrect) — used by the archive panel's per-row "Restore"
 * button, as opposed to resetConfirmedCorrect() which clears everything
 * at once. Takes the hash token directly (as returned by hashConfirmedPair
 * / shown to the archive endpoint) rather than re-deriving it from
 * (issueType, text), since the archive UI already has the token from
 * when it built the list.
 */
export async function unmarkIssueCorrect(docId, accessToken, hashToken) {
  const auth = authorizedClient(accessToken);
  const drive = google.drive({ version: "v3", auth });
  const { data } = await drive.files.get({ fileId: docId, fields: "appProperties" });
  const existingTokens = new Set((data.appProperties?.moConfirmedCorrect || "").split(",").filter(Boolean));
  existingTokens.delete(hashToken);

  await drive.files.update({
    fileId: docId,
    requestBody: { appProperties: { moConfirmedCorrect: [...existingTokens].join(",") } },
  });
}

/**
 * Clears every confirmed-correct instance for this document. Called
 * alongside resetDismissedIssues() as one combined "reset all my
 * overrides for this doc" action (see index.js's /reset-dismissed-issues)
 * rather than a separate endpoint, since from the reviewer's perspective
 * both are "forget what I've already confirmed about this document."
 */
export async function resetConfirmedCorrect(docId, accessToken) {
  const auth = authorizedClient(accessToken);
  const drive = google.drive({ version: "v3", auth });
  await drive.files.update({
    fileId: docId,
    requestBody: { appProperties: { moConfirmedCorrect: "" } },
  });
}

/**
 * Un-mutes a single previously-dismissed issue TYPE (the inverse of
 * dismissIssueType) — used by the archive panel's per-row "Restore"
 * button, as opposed to resetDismissedIssues() which clears everything
 * at once.
 */
export async function undismissIssueType(docId, accessToken, issueType) {
  const code = DISMISSIBLE_ISSUE_CODES[issueType];
  if (!code) {
    throw new Error(`"${issueType}" isn't a dismissible issue type.`);
  }
...
function findHighlightedRanges(content) {
  ...
  for (const el of content || []) {
    if (el.paragraph) {
      ...
      // Same reasoning as extractRuns()'s walkContent(): the signatory
      // block — exactly where most of Mo's highlights land — is a table,
      // so without recursing into cells, highlights placed there would
      // never be found here and clearAllMoHighlights() would silently
      // leave old highlight residue behind on every re-check.
      for (const row of el.table.tableRows || []) {
        for (const cell of row.tableCells || []) {
          ranges.push(...findHighlightedRanges(cell.content));
        }
      }
    }
  }
