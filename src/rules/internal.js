...
import { checkWitnessethFieldFormatting, checkInternalGtcFieldFormatting } from "./shared.js";

export function checkInternal(fullText, { runs } = {}) {
  const issues = [];
  ...
  if (!fullText.includes("Witnesseth that:")) {
    ...
    // DLSU-OFFICE-SHORT ORGANIZATION NAME / EVENT NAME / dates / venue must each be bold + ALL CAPS
    issues.push(...checkWitnessethFieldFormatting(fullText, runs));
  }
  ...
  // GTC clause 1 (org names) and clause 2 (event name) fields must each
  // be bold + ALL CAPS. Internal's GTC only has 2 clauses (confirmed
  // against the actual template — unlike Sponsorship/Partnership's 4),
  // with its own distinct wording, so this is separate from
  // checkGtcFieldFormatting() in shared.js.
  issues.push(...checkInternalGtcFieldFormatting(fullText, runs));
