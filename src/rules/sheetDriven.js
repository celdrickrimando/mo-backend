...
/**
 * Parses the "Constant Signatories" tab: people whose name+position pair
 * should be spelled/formatted consistently across every MOA (e.g. the
 * Dean of Student Affairs, Director for SLIFE) as opposed to
 * per-sponsorship-amount tiers. Column layout (see
 * CANONICAL_SHEET_SETUP.md): A is a spacer, B is Name, C is Position (as
 * it reads on the first page, position-before-name), D is Position (as
 * it reads in the signatory block, name-before-position). C and D are
 * independent — a person may only appear on the first page, only in the
 * signatory block, or both, so an empty C or D just means "don't check
 * that spot for this person" rather than an error.
 *
 * Returns [] if the tab has no rows — caller falls back to the hardcoded
 * defaults in rules/constantSignatories.js, same pattern as
 * getSignatoryTiersFromSheet() above.
 */
export function getConstantSignatoriesFromSheet(rulesConfig) {
  const rows = (rulesConfig?.constantSignatories || []).filter((r) => isActive(r) && (r["name"] || "").trim());
  if (rows.length === 0) return [];

  return rows.map((r) => ({
    name: (r["name"] || "").trim(),
    positionFirstPage: (r["position (first page)"] || "").trim() || null,
    positionSignatory: (r["position (signatory)"] || "").trim() || null,
  }));
}
